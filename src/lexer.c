...
void lexer_tag_clean(string_t *s)
{
	if (s->buf[0] == '{' && s->buf[1] == '%' &&
...
void lexer_obj_clean(string_t *s)
{
	if (s->buf[0] == '{' && s->buf[1] == '{' &&
	    s->buf[s->len - 2] == '}' && s->buf[s->len - 1] == '}') {
		s->buf[0] = ' ';
...
int lexer_block_add(fluid_t *ctx, enum lexer_block_e type,
